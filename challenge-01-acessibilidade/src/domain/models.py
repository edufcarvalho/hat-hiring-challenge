...
class Gasto(SQLModel, table=True):
  ...
  orgao_id: UUID = Field(default=None, foreign_key="orgaos.id", description="ID do órgão responsável pelo gasto")
  categoria_id: UUID = Field(default=None, foreign_key="categorias.id", description="Categoria do gasto")
  ...
  favorecido_id: UUID = Field(default=None, foreign_key="favorecidos.id", description="ID do favorecido do gasto")
...
class Orgao(SQLModel, table=True):
  ...
  id: UUID = Field(default_factory=uuid8, primary_key=True)
  nome: str = Field(unique=True, index=True, description="Nome do ministério ou órgão")
  sigla: Optional[str] = Field(default=None, description="Acrônimo do ministério ou órgão")
...
class Favorecido(SQLModel, table=True):
  ...
  tipo: TipoPessoa = Field(default=None, description="Tipo do favorecido (ex: pessoa física, empresa, etc.)")
